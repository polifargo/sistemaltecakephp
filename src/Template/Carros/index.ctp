    ...
    <div class="col-xs-12">
      <div class="box">
        <div class="box-header">
          <h3 class="box-title"><?= __('Lista de') ?> Carros</h3>
          <div class="box-tools">
          ...
          <table class="table table-hover">
            ...
            <?php foreach ($carros as $carro): ?>
              <tr>
                ...
                <td><?= h($carro->nome) ?></td>
                <td><?= $carro->has('categorias_carro') ? $this->Html->link($carro->categorias_carro->nome, ['controller' => 'CategoriasCarros', 'action' => 'view', $carro->categorias_carro->id]) : '' ?></td>
                <td><?= h($carro->transmissao) ?></td>
                <td><?= h($carro->cor) ?></td>
                <td><?= h($carro->combustivel) ?></td>
                <td><?= $this->Number->format($carro->qtd_portas) ?></td>
                <td class="actions" style="white-space:nowrap">
                  <?= $this->Html->link(__('Visualizar'), ['action' => 'view', $carro->id], ['class'=>'btn btn-info btn-xs']) ?>
                  <?= $this->Html->link(__('Editar'), ['action' => 'edit', $carro->id], ['class'=>'btn btn-warning btn-xs']) ?>
                  <?= $this->Form->postLink(__('Deletar'), ['action' => 'delete', $carro->id], ['confirm' => __('Confirm to delete this entry?'), 'class'=>'btn btn-danger btn-xs']) ?>
                </td>
              </tr>
            <?php endforeach; ?>
            </tbody>
